                                ...
                                conexion.query('SELECT * FROM auth WHERE email = ?', [req.body.email], (err, auth) => {
                                    if(err) return res.json({ meesage: 'Algo salio mal con la query', err: err.sqlMessage })
                                        conexion.query('INSERT INTO patrocinadores SET ?', [{
                                            ...
                                            conexion.query('SELECT * FROM patrocinadores WHERE auth = ?', [auth[0].id], async (err, user) => {
                                                const token = await jwt.sign({ id: user[0].id }, process.env.SECRET_KEY, {
                                                    expiresIn: process.env.JWT_EXPIRE,
                                                });
                                                return res.cookie('token', token ).json({ success: true, message: 'Patrocinador registrado', user: {token: token, nombre: user[0].nombre} })
                                            })
                                        })
                                });
                            });
