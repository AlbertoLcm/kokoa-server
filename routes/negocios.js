                                        ...
                                        if(err) return res.json({ meesage: 'Algo salio mal con la query', err: err.sqlMessage })
                                            conn.query('INSERT INTO negocios SET ?', [{
                                                ...
                                                conn.query('SELECT * FROM negocios JOIN auth ON negocios.auth = auth.id WHERE auth.id = ?', [auth[0].id], async (err, user) => {
                                                    ...
                                                    return res.cookie('token', token ).json({ success: true, message: 'Negocio registrado', user: {token: token, data: user} })
                                                })
                                            })
                                    });
                                });
...
routes.put('/:id', (req, res) => {
    req.getConnection((errBD, conn) => {
        ...
        conn.query('UPDATE negocios set ? WHERE id = ?', [req.body, req.params.id], (err, result) => {
            if(err) return res.send(err)
