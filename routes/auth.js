...
routes.post('/login', async (req, res) => {
    try {
        req.getConnection((err, conn) => {
            ...
            conn.query(`SELECT * FROM auth WHERE email = ? or telefono = ?`, [req.body.email, req.body.email], async (err, usuario) => {
                if(err) return res.json({message: "algo salio mal en la query", error: err});
                ...
                if(usuario[0] !== undefined){
                    ...
                    if(!buscarPass){
                        ...
                        conn.query('SELECT * FROM usuarios WHERE auth = ?', [usuario[0].id], async (err, user) => {
                            if(err) return res.json({message: "algo salio mal en la query", error: err});
