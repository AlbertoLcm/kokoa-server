...
routes.post('/add', (req, res) => {
  ...
  try {
    ...
    req.getConnection((errBD, conn) => {
      ...
      conn.query('SELECT * FROM eventos where lat = ? and lng = ?', [
        ...
        if (!eventos.length) {
          conn.query('INSERT INTO eventos SET ?', [
            ...
            // conn.query('SELECT * FROM usuarios JOIN auth WHERE usuarios.auth = auth.id', (err, usuarios) => {
            //   if (err) 
            //     return res.json({msg: err})
...
routes.get('/all/:id', (req, res) => {
  req.getConnection((errBD, conn) => {
    ...
    conn.query('SELECT * FROM eventos JOIN auth ON eventos.anfitrion = auth.id WHERE auth.id = ?', [req.params.id], (err, eventosBD) => {
      if (err) 
        return res.send(err)
...
routes.get('/:id', (req, res) => {
  req.getConnection((errBD, conn) => {
    ...
    conn.query('SELECT * FROM eventos JOIN auth ON eventos.anfitrion = auth.id WHERE eventos.id_evento = ?', [req.params.id], (err, eventoBD) => {
      if (err) 
        return res.send(err)
      ...
      conn.query(`SELECT * FROM ${
        eventoBD[0].rol
      } WHERE auth = ?`, [eventoBD[0].anfitrion], (err, anfitrionBD) => {
        if (err) 
          return res.status(400).json({message: 'Algo salio mal con la Query', error: err})
